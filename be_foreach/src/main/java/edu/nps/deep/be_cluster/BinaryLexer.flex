...
%{
  ...
  // the reader typecast to the SplitReader that it is
  final private SplitReader splitReader;
  ...
  boolean at_eof(){
    return zzAtEOF || (count > splitSize);
  }

%}

%init{
splitReader = (SplitReader)in;
...
//System.out.println("zzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzz feature found");
              features.add(new Feature(Long.toString(count + splitOffset), email, splitReader.readContext((int)count, yylength())));
              count += yylength();
            }
